...
Almost aspects of the rendering can be opt-in, with replacing syntax allowed, and significantly, because each aspect is downloaded within an overridable method, overriding the method and establishing alternative syntax has no penalty from the original syntax.

The core rendering logic is also kept quite minimal The core transform syntax xtal-element relies on, [DTR](https://github.com/bahrus/trans-render#declarative-trans-render-syntax-via-plugins),does its job well, but is limited to providing property value distributing, and adding event listeners, common use case conditional logic.  But it stays true to our definition of declarative -- where no side effects can introduced, and can execute anywhere html can execute, just like web components.

It stops there, and doesn't even provide support looping, which most other UI libraries provide.
...
2.  The "Concessional Branch":  CSS styling, imported via CSS Modules.  There are two strong reasons to keep the CSS in a separate file (but this is not a doctrinaire rule).  
    ...
    2.  "Dependency injection":  with the help of the link preload tag (and/or, possible, import maps), allow the consumer of the web component to define their own theme, with no penalty for the default theme if no theme is provided.
