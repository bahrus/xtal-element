...
export function letThereBeProps(elementClass, slicedPropDefs, callbackMethodName) {
    ...
    if (proto.xtalIgnoreFalsy === undefined)
        proto.xtalIgnoreFalsy = new Set();
    if (proto.xtalIgnoreTruthy === undefined)
        proto.xtalIgnoreTruthy = new Set();
    const existingProps = Object.getOwnPropertyNames(proto);
    const props = slicedPropDefs.propDefs;
    for (const prop of props) {
        if (prop.stopReactionsIfFalsy)
            proto.xtalIgnoreFalsy.add(prop.name);
        if (prop.stopReactionsIfTruthy)
            proto.xtalIgnoreTruthy.add(prop.name);
